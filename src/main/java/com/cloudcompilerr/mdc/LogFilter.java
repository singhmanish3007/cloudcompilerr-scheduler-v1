package com.cloudcompilerr.mdc;
...
public class LogFilter extends OncePerRequestFilter {
	...
	private Set<String> maskedHeadersSet = new HashSet<>(
			LogConstants.MANDATORY_MASKED_HEADERS);
	private Clock clock;
	...
	protected void doFilterInternal(HttpServletRequest request,
		...
		String trackingId = request
				.getHeader(LogConstants.HTTP_HEADER_TRACKING_ID);
		if (trackingId == null) {
		...
		try {
			response.addHeader(LogConstants.HTTP_HEADER_TRACKING_ID, URLEncoder
					.encode(trackingId, StandardCharsets.UTF_8.name()));
		...
		try {
			...
			MDC.put(LogConstants.API_RESPONSE_STATUS,
					Integer.toString(response.getStatus()));
			MDC.put(LogConstants.API_RESPONSE_TIME,
					Long.toString(clock.millis() - startTime));
		}
	}

	protected void addMetadataToMDC(HttpServletRequest request,
			Long startTime) {
		...
		MDC.put(LogConstants.API_KEY, ServletRequestUtils
				.getStringParameter(request, LogConstants.API_KEY, ""));
		MDC.put(LogConstants.API_METHOD,
				(String) request.getAttribute(LogConstants.API_METHOD_NAME));
		if (null != request.getQueryString()
				&& !"".equals(request.getQueryString().trim())) {
			MDC.put(LogConstants.API_REQUEST_URI, String.format("%s?%s",
					request.getRequestURI(), request.getQueryString()));
		} else {
			MDC.put(LogConstants.API_REQUEST_URI,
					String.format("%s", request.getRequestURI()));
		}
	}
	...
	protected Map<String, Object> extractHeadersToMap(
	...
	protected Map<String, Object> extractHeadersToMap(
		...
		response.getHeaderNames().stream().forEach(headerName -> {
			HeaderEntry headerEntry = getKVMapForHeader(headerName,
					(List) response.getHeaders(headerName));
			headers.put(headerEntry.getKey(), headerEntry.getValue());
		});
	...
	private HeaderEntry getKVMapForHeader(String headerName,
			List<String> headers) {
